...
std::string uncompress_string(const std::string& str)
{
    z_stream zs;                        // z_stream is zlib's control structure
    memset(&zs, 0, sizeof(zs));
...
void Adapter::Xaction::start() {
    ...
    this->gzippedContent = false;
    
    if(adapted->header().hasAny(contentEncodingName)) {
        const libecap::Header::Value contentEncoding = adapted->header().value(contentEncodingName);
	
        if(contentEncoding.size > 0) {
            std::string contentEncodingString = contentEncoding.toString();
            
            if(strstr(contentEncodingS.c_str(),"gzip")) {
                this->gzippedContent = true;
            }
        }
    }
...
void Adapter::Xaction::adaptContent(std::string &chunk) const {
    ...
    //this->contentNotAdopted = false;
    
    if (gzippedContent) {
        std::string plain = uncompress_string(chunk);
        
        if ((pos = plain.find(victim)) != std::string::npos) {
            plain.replace(pos, victim.length(), replacement);
            chunk.clear(); 
            chunk = compress_string(plain);
        } else {
            //this->contentNotAdopted = true;
        }
            
        } else { // text/html
         
        if ((pos = chunk.find(victim)) != std::string::npos) {
